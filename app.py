...
with analytic_s:
    ...
    sns.set(style="darkgrid") 
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.countplot(x='Type_of_order', data=dataset,ax=ax)
    ax.set_xticklabels(['Snack','Meal','Drinks','Buffet'])
    ax.set_ylabel('Number of Orders')
    st.pyplot(fig)
    ...
    sns.set(style="darkgrid") 
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.countplot(x='Weatherconditions', data=dataset,ax=ax)
    ax.set_xticklabels(['Fog','Stormy','Cloudy','Sandstorms','Windy','Sunny'])
    ax.set_ylabel('Number of Orders')
    st.pyplot(fig)
    ...
    sns.set(style="darkgrid") 
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.countplot(x='Type_of_vehicle', data=dataset,ax=ax)
    ax.set_xticklabels(['motorcycle','scooter','electric scooter'])
    ax.set_ylabel('Number of Orders')
    st.pyplot(fig)
    ...
    sns.set(style="darkgrid") 
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.countplot(x='Road_traffic_density', data=dataset,ax=ax)
    ...
    st.write('Food preperation time vs. Type of order')
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.boxplot(x='Type_of_order', y='Food_Preparation_time_minutes', data=dataset, ax=ax)
    ax.set_xticklabels(['Snack','Meal','Drinks','Buffet'])
    st.pyplot(fig)

    st.write('Delivery Time vs. Type of order')
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.boxplot(x='Type_of_order', y='Time_taken(min)', data=dataset, ax=ax)
    ax.set_xticklabels(['Snack','Meal','Drinks','Buffet'])
    st.pyplot(fig)

    dataset['Order_Date'] = pd.to_datetime(dataset['Order_Date'])
    count_df = dataset.groupby(dataset['Order_Date'].dt.to_period("M")).size().reset_index(name='Number of Orders')
    count_df['Order_Date'] = count_df['Order_Date'].dt.strftime('%Y-%m')
    fig = px.line(count_df, x='Order_Date', y='Number of Orders', title='Time Series Plot', markers=True)
    st.plotly_chart(fig)

    
    dataset['Day_of_Week'] = dataset['Order_Date'].dt.dayofweek
    count_df = dataset.groupby('Day_of_Week').size().reset_index(name='Number_of_Orders')
    count_df['Day_of_Week'] = count_df['Day_of_Week'].map({0: 'Monday', 1: 'Tuesday', 2: 'Wednesday', 3: 'Thursday', 4: 'Friday', 5: 'Saturday', 6: 'Sunday'})
    fig = px.bar(count_df, x='Day_of_Week', y='Number_of_Orders', title='Number of Orders by Day of the Week', labels={'Number_of_Orders': 'Number of Orders'})
    st.plotly_chart(fig)

    fig = px.line(dataset.groupby('Day_of_Week')['Time_taken(min)'].mean().reset_index(), 
              x='Day_of_Week', y='Time_taken(min)', 
              title='Average Time Taken by Day of the Week',
              labels={'Time_taken(min)': 'Average Time Taken (min)'})
    fig.update_layout(
        xaxis=dict(
            tickmode='array',
    ...
    st.write('Festival vs. Time Taken')
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.boxplot(x='Festival', y='Time_taken(min)', data=dataset, ax=ax)
    ax.set_xticklabels(['No','Yes'])
    st.pyplot(fig)

    st.write('Vehicle condition vs. Time Taken')
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.boxplot(x='Vehicle_condition', y='Time_taken(min)', data=dataset, ax=ax)
    #ax.set_xticklabels(['Yes','No'])
    st.pyplot(fig)
    
    st.write('Box Plot of Road Traffic Density vs. Time Taken')
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.boxplot(x='Road_traffic_density', y='Time_taken(min)', data=dataset, ax=ax)
    ax.set_xticklabels(['Low', 'Jam', 'Medium', 'High'])
    st.pyplot(fig)
